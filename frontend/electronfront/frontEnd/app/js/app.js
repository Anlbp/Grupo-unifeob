...
async function excluirVenda(id) {
  if (!temPermissao()) {
    showToast("Acesso negado", "Você não tem permissão para excluir");
    return;
  }
  ...
  if (confirm("Deseja realmente excluir esta venda?")) {
    try {
      const res = await window.api.deleteVenda(id, token);
      if (res.ok) {
        showToast("Sucesso", "Venda excluída");
        carregarVendas();
      } else {
        showToast("Erro", res.message || "Falha ao excluir");
      }
    } catch (err) {
      console.error(err);
      showToast("Erro", "Erro de comunicação");
    }
  }
}
...
async function editarVenda(id) {
  if (!temPermissao()) {
    showToast("Acesso negado", "Você não tem permissão para editar");
    return;
  }

  const token = localStorage.getItem("token");
  try {
    const venda = await window.api.getVenda(id, token);
    console.log("Venda para editar:", venda);
    // Aqui você pode abrir um modal e preencher os campos com os dados da venda
  } catch (err) {
    console.error(err);
    showToast("Erro", "Erro ao buscar dados da venda");
  }
}
